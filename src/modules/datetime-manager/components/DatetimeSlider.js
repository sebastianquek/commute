...
class DatetimeSlider extends React.Component {
  ...
  drawSlider () {
    ...
    const w = this.w
    const h = this.h / 2
    const padding = { top: 14, right: 140, bottom: 14, left: 60 }
    ...
    // Define zoom
    // Uses max zoom out of 1 week
    const zoom = d3.zoom()
      .scaleExtent([
        (xScale.range()[1] - xScale.range()[0]) / (xScale(moment('2016-07-17T00:00:00').toDate()) - xScale(moment('2016-07-10T00:00:00').toDate())),
        20
      ])
      .translateExtent([
        [0, 0],
