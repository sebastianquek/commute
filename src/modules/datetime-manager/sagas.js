...
import { REQUEST_RIDERSHIP } from './actionTypes'
import { minDateSelector, maxDateSelector, stepSelector } from './selectors'
import zoneManager from '../zone-manager'

export function * watchAndUpdateRidership () {
  while (true) {
    const { id } = yield take([REQUEST_RIDERSHIP, zoneManager.actionTypes.ADD_SELECTION])
    ...
    if (id) { // Get specific zone's journeys
      zoneIds.push(id)
    } else { // Get journeys for all currently selected zones
      zoneIds = yield select(zoneManager.selectors.allZoneIdsSelector)
    }

    try {
      const journeys = yield call(fetchRidership, zoneIds, minDate, duration, step)
